---
title: three.js render targets in/out
description: Zero-copy denoising of three.js WebGPURenderer render targets — get a GPUTexture in, get a GPUTexture out, no CPU pixels anywhere.
nav: 20
---
...
working example
([`examples/three-pathtracer-webgpu/src/main.ts`](https://github.com/pmndrs/denoiser/blob/main/examples/three-pathtracer-webgpu/src/main.ts)).
...
> [!IMPORTANT]
> The order is fixed: **denoiser first, then hand its device to three.js.**
...
| size | anything; up to `maxRunPixels` (default 1080p-ish) runs whole-frame in one model pass, larger tiles automatically — see [Large images & tiling](./large-images-and-tiling) |
...
view-dependent — re-render it when the camera moves. For the full set of
conventions (env-pixel normals, HDR albedo clamping, first-hit rules) see
[Generating aux inputs](./generating-aux-inputs).
...
See the [API reference](../reference/api) for the exhaustive, always-current version of this table.
...
> [!NOTE]
> sRGB texture formats can't be `STORAGE_BINDING` — a caller-owned output
...
More failure modes (black output from a device mismatch, CORS, device-lost) are
covered in [Troubleshooting](./troubleshooting).

Live: [three-pathtracer-webgpu](https://pmndrs.github.io/denoiser/three-pathtracer-webgpu/) · [ldraw-eiffel](https://pmndrs.github.io/denoiser/ldraw-eiffel/)
